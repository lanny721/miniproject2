...
void Enemy::Hit(float damage) {
    InvisibleEnemy* invisibleEnemy=dynamic_cast<InvisibleEnemy*>(this);
    if(invisibleEnemy && !invisibleEnemy->IsVisible()){
        damage = 0;
    }
    hp -= damage;
    if (hp <= 0) {
...
void Enemy::Update(float deltaTime) {
    if(isFrozen) {
        ...
        if (freezeTime <= 0) {
            ...
            TankEnemy* tank = dynamic_cast<TankEnemy*>(this);
            if (tank) {
                tank->SetHead(255, 255, 255, 255);
            }
        }
        return;
    }
...
void Enemy::Freeze() {
    if(!isFrozen){
        isFrozen = true;
        freezeTime = 3.0f;
        Tint = al_map_rgba(155, 155, 255, 255);

        TankEnemy* tank = dynamic_cast<TankEnemy*>(this);
        if (tank) {
            tank->SetHead(155, 155, 255, 255);
        }
    }
}
