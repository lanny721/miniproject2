...
#include "Enemy/InvisibleEnemy.hpp"
    ...
    for (int i = 0; i < SpeedMult; i++) {
        ...
        switch (current.first) {
            ...
            case 3:
                ...
                break;
            case 4:
                EnemyGroup->AddNewObject(enemy = new InvisibleEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
                break;
            default:
                continue;
        }
...
void PlayScene::OnKeyDown(int keyCode) {
    ...
    if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        for(auto it = keyStrokes.begin(); it != keyStrokes.end(); it++) {
            if (*it == ALLEGRO_KEY_LSHIFT && code[i] == ALLEGRO_KEY_RSHIFT) {
                ++i;
                continue;
            }
            if (*it != code[i]){
        ...
        if (cheated!=0 && keyStrokes.size() == code.size()) {
            // Cheat code entered.
            //Engine::Point pos = Engine::GameEngine::GetInstance().GetMousePosition();
            Engine::IObject* plane=new Plane();
    ...
    if (keyCode == ALLEGRO_KEY_Q) {
        ...
        // Hotkey for Speed up.
        SpeedMult = keyCode - ALLEGRO_KEY_PAD_0;
    }
}
void PlayScene::Hit() {
    lives--;
